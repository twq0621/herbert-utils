package gear.net {
	...
	public class SWFLoader extends RESLoader {
		...
		private function completeHandler(event : Event) : void {
			...
			_isLoadding = false;
			_isLoaded = true;
			swfHeight = loaderInfo.height;
			swfWidth = loaderInfo.width;
		 ...
		 * @param className 类名称
		 * @return 		 */
		public function getAutoScaleSkin(asset : AssetData) : Sprite {
			var assetClass : Class = getClass(asset.className);
			if (assetClass == null) {
				return null;
			}
			var result : * = new assetClass();
			if (result is BitmapData) {
				return  new ScaleBitmapSprite(result, asset.scaleBitmapRect);
			} else if (result is Sprite) {
				return Sprite(result);
			} else {
				return null;
