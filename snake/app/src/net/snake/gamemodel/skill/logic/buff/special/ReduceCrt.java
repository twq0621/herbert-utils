 ...
 *
 */
public class ReduceCrt extends Buff {

//	private static final Logger logger = Logger.getLogger(ReduceCrt.class);
	
	public ReduceCrt(EffectInfo effectInfo) {
		super(effectInfo);
	}
...
//					logger.warn("enter(EffectController) - 不存在的CharacterSkill:{}", effect.getRelevanceSkillId()); //$NON-NLS-1$
				...
				if (AttackFormula.bangpaiSkillLv(characterSkill.getRealGrade(), affecter.getUnCrtGrade())) {
					int durationTime = AppEventCtlor.getInstance().getEvtSkillFormula().bangpaiSkillDuration(characterSkill.getRealGrade(),affecter.getUnCrtGrade());
		...
		controller.setReduceCrtBuff(true);
	...
	public boolean leave(EffectController controller) {
		controller.setReduceCrtBuff(false);
		VisibleObject affecter = effect.getAffecter();
		affecter.getPropertyAdditionController().recompute();
		return true;
	}
