...
public class Shouhuxuxian_SceneLoop implements IEventListener {

	private static int[] pos1 = { 21, 17 };
	private static int[] pos2 = { 30, 67 };
	private static int[] pos3 = { 115, 67 };

	private static final Logger logger = Logger.getLogger(Shouhuxuxian_SceneLoop.class);

	private static int[] monsterIds = { 70100281, 70100281, 70100281, 70100281, 70100281, 70100281, 70100281, 70100281, 70100281 };
		...
		if (now >= timestamp) {
			...
			if (counter == 9) {
				...
				if (size == 1) {
					...
					SMonster sceneMonster = api.createMonsterToScene(71000313, pos1[0], pos1[1], 7, 1, 1, false, scene);
