package gear.net {
	...
	public final class RESManager extends EventDispatcher {
		...
		public static function getSkin(asset : AssetData) : Sprite {
			...
			if (asset.scaleBitmapRect) {
				return loader.getAutoScaleSkin(asset);
			} else {
		...
		public static function getMCStage(key : String) : MovieClip {
			var loader : SWFLoader = _loaded[key] as SWFLoader;
			return loader.content as MovieClip;
		}

		public static function getSWFStageInfo(key : String) : Rectangle {
			var loader : SWFLoader = _loaded[key] as SWFLoader;
			return new Rectangle(0, 0, loader.swfWidth, loader.swfHeight);
		}
		...
		/**
		 * key也如同getByteArray方法的key。
		 */
		public static function getAmf3Data(key : String, registerUrl : String, ...args : Array) : * {
			var loader : RESLoader = _loaded[key] as RESLoader;
			if (loader == null) {
				return null;
			}
			var index : uint;
			for each (var register : Class in args) {
				registerClassAlias(registerUrl + index, register);
				index++;
			}
			var byteArray : ByteArray = loader.getByteArray();
