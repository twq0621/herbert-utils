...
import net.snake.gamemodel.rankings.response.RankingResponse50304;
 ...
 * 
 */
@MsgCodeAnn(msgcode = 50303, accessLimit = 100)
public class RankingGradeProcess50303 extends CharacterMsgProcessor implements IThreadProcessor {
	@Override
	public void process(Hero character, RequestMsg request) throws Exception {
		...
		if (null != list) {
			...
			for (CharacterRanking character2 : list) {
				jishu = jishu + 1;
				if (character.getId().intValue() == character2.getId()) {
					mingci = jishu;
					break;
				}
			}
		...
		RankingResponse50304 response50304 = new RankingResponse50304(list, mingci, pagecount);
		character.sendMsg(response50304);
