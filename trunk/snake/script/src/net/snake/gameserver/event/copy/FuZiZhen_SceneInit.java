 ...
 * 
 */
public class FuZiZhen_SceneInit extends SuperInstance implements IEventListener {
	...
	public void handleEvent(SApi api, Object[] args) {
		...
		// 得到副本的怪物配置
		Collection<SMonster> collection = scene.getSMonsterCollection();
		// 如果怪物的配置 少于 约定的 组数 则无法开启副本
		if (collection == null || collection.size() != groupNum + 1) {
			return;
		}
		...
		int size = GlobalLanguage.str_idioms.length;
		for (int i = 0; i < size; i++) {
			idioms.add(GlobalLanguage.str_idioms[i]);
		}
		instance.setAttribute("idioms", idioms);
		// 10 秒后刷 第一波 怪物
