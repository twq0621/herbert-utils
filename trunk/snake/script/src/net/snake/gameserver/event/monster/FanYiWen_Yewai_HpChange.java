...
public class FanYiWen_Yewai_HpChange implements IEventListener {
	...
	public void handleEvent(SApi api, Object[] args) {
		...
		if (monster.getModel() == MonsterID) {
			if (monster.getHpPercent() > 0.2) {// 2.
												// 受到攻击时一定30%几率使用大范围伤害技能（技能等级=60）
				if (Math.random() < 0.3) {
					api.restoreMonsterHp(monster, 300);
				}
