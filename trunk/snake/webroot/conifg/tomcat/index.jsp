...
<html>
  <head>
    ...
    <h1>这给是简体打包---------</h1>
  		下边是当前执行的模版type是0的话就是执行完了1的话就是正在执行中<br>
...
setTimeout("reflesh()",10*1000);//每10秒钟刷新一次
 ...
 <h1>美工上传玩地图之后请点击提交上传地图</h1>
  <h1><a href="/antcall/ant?task=1"  target="_blank">提交上传地图</a><br>	</h1>
  

  <h1>策划导完数据之后请点击一下同步数据之后在点击下面的xml生成</h1>
  <h1><a href="/antcall/ant?task=sql"  target="_blank">策划导完数据之后请点击一下同步数据</a><br>	</h1>
  <body>
  		<a href="/antcall/ant?task=banyun"  target="_blank">同步各个版本的avatar人物换装，avatar_effect效果 还有 ico图标文件 注意需要手动去对应工程目录提交  </a><br>	
	<br/>
    <br/>
    <a href="/antcall/ant?task=lianti"  target="_blank">lianti人物连体（这个不再生成所有配置xml里边需要单独生成</a><br>
     <br/>
       <a href="/antcall/ant?task=dgwd"  target="_blank">dgwd人物独孤问道（这个不再生成所有配置xml里边需要单独生成</a><br>
     <br/>
     
     <a href="/antcall/ant?task=languageflash"  target="_blank">lan_text（这个不再生成所有配置xml里边需要单独生成</a><br>
     <br/>
     <a href="/antcall/ant?task=shopshengwang"  target="_blank">shopshengwang（这个不再生成所有配置xml里边需要单独生成</a><br>
